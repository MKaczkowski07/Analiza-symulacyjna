...
# Zakładam, że dane_final już istnieje i ma kolumny:
# Country, Smokers, Population, GDP_per_capita, Akcyza, Wydatki_na_medycyne

dane_final <- dane_final %>%
  mutate(
    Health_pc = Wydatki_na_medycyne / Population
  )
...
var_labels <- c(
  ...
  Health_pc      = "Wydatki med. per capita"
)
...
server <- function(input, output, session) {
  ...
  output$model_summary <- renderPrint({
    # Budujemy formułę w zależności od checkboxów
    predictors <- c()
