...
-Eurostat (w postaci plików xlsx)
```r
dane1 <- read_excel("Tobacco_consumption_HLTH2022-2.xlsx", sheet = "T1")
OdsetekPalacy <- dane1[, 1:2] %>%
  arrange(.[[1]]) %>%
...
- **Wydatki medyczne**: Eurostat - całkowite wydatki na ochronę zdrowia w mln 
...
-Wysokość akcyzy nakładanej na paczke papierosów
...
### Zamożność a zdrowie
Bogatsze kraje mają **nieco niższy odsetek palących** (r = `r round(cor3, 2)`), co może wynikać z wyższej świadomości zdrowotnej.
</div>

<div style="background: #fbeeee; padding: 15px; border-radius: 8px; border-left: 4px solid #e74c3c;">

### Wydatki medyczne
Wydatki medyczne per capita korelują z odsetkiem palących. (r = `r round(cor4, 2)`).
</div>
...
- Dane pochodzą z różnych źródeł (Eurostat, Tax Foundation), co może powodować różnice w metodologii pomiaru.
- Analiza opiera się na jednym roku (2019), przez co nie uwzględnia trendów czasowych ani efektów opóźnień.
- Nie uwzględniono czynników społecznych i kulturowych (np. edukacji, kampanii antynikotynowych), które również wpływają na odsetek palących.
- Wydatki medyczne i akcyza są wartościami zagregowanymi na poziomie kraju - nie analizujemy struktury wydatków.
- Model regresji wielorakiej wyjaśnia tylko część zmienności (≈ `r round(summary(model)$r.squared * 100,1)`%), co oznacza istnienie innych silnych determinant palenia.
...
## Kompleksowe podejście i zróżnicowanie regionalne

Skuteczna polityka antytytoniowa powinna łączyć **instrumenty fiskalne** (akcyza),
**działania edukacyjne** oraz **programy wsparcia** dla osób chcących rzucić palenie.

Duże różnice regionalne w Europie wskazują jednak, że polityka zdrowotna
musi być **dostosowana do specyfiki kulturowej i społecznej** poszczególnych krajów.
...
<br>
<br>
Analiza zapisów akcyzowych, zachowań zdrowotnych i wskaźników ekonomicznych  
pokazuje, że polityka fiskalna może odgrywać kluczową rolę w kształtowaniu zdrowia publicznego.  
...
Filip Kasprzycki &nbsp;•&nbsp; Mateusz Kaczkowski  
</div>
